...
router.post('/', requireAuth, (req, res) => {
  ...
  sendEmail({
    to: client_email,
    subject: `Invoice ${invoiceNumber} from your service`,
    html: `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px">
      <h1 style="color:#6366f1;margin-bottom:16px">Invoice ${invoiceNumber}</h1>
      <p>Hi ${client_name},</p>
      <p>You have a new invoice for <strong>${total.toFixed(2)}</strong>.</p>
      <p style="margin:24px 0">
        <a href="${appUrl}/i/${shareToken}" style="background:#6366f1;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none">
          View Invoice
        </a>
      </p>
      <hr style="border:none;border-top:1px solid #eee;margin:24px 0" />
      <p style="color:#888;font-size:13px">InvoicePal</p>
    </div>`
  });
